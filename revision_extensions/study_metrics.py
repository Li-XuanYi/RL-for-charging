...
def metrics(rows, decisions, goal_time, failure_reason, soc_max=.95):
    ...
    # Same sampled metric definitions as E2, generalized to the actual cell count.
    n_cells=max(r['cell'] for r in rows)
    if n_cells<3:
        raise ValueError('Experiments 3-6 require at least three cells')
    by_cell = [[r for r in rows if r['cell'] == i] for i in range(1,n_cells+1)]
    ...
    limits_respected = bool(finite and np.max(v_excess) <= 1e-5 and np.max(t_excess) <= 1e-5
                       and np.max(soc_excess) <= 1e-7 and np.min(allsoc) >= -1e-7
                       )
    audit_complete=not bool(failure_reason) and not any(d.get('post_switch_peak_unknown',False) for d in decisions)
    constraints=limits_respected and audit_complete
    ...
    return {
        ...
        'n_agents':n_cells,'final_charge_soc_vector':soc[-1].tolist(),
        ...
        'sampled_physical_limits_respected':limits_respected,'constraint_audit_complete':audit_complete,
        'operational_failure':bool(failure_reason),
        ...
        'reward_penalty_activation_steps':{key:sum(abs(d.get('reward_terms',{}).get(key,0.))>1e-12 for d in charge_decisions)
                                           for key in ('time','balance','voltage','temperature')},
...
def plot_case(rows, path, title):
    ...
    for i in range(1,max(r['cell'] for r in rows)+1):
