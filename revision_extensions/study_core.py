...
DEFAULTS=dict(seeds=[7,17,27,37,47],decision_s=90.,horizon_s=5400.,hold_s=270.,
    ...
    compress_trajectories=True,replicate_training_triplet=False,
...
def code_fingerprint():
    ...
    return {str(p.relative_to(paths.ROOT)):sha256(p) for p in files}
...
def _completed(root,number,cfg):
    ...
    if explicit:folder=inside(Path(explicit),base)
    else:
        ...
        folder=inside(Path(complete.read_text(encoding='utf-8-sig').strip()),base)
...
def load_chain(root,out,cfg,required=(1,2)):
    ...
    if Path(source['source']).resolve()!=one.resolve() or source['sha256']!=sha256(one/'handoff.json'):
        raise ValueError('Selected E1 and E2 do not share the same calibration')
    assessment=json.loads((e2/'scientific_assessment.json').read_text(encoding='utf-8-sig'))
    ...
    for number,folder in upstreams.items():
        ...
        for dependency,path in lineage['upstreams'].items():
            dep=int(dependency)
            if dep in upstreams and Path(path).resolve()!=upstreams[dep].resolve():
...
def load_policy(entry):
    policy=make_policy(entry['spec'],entry['n_agents'],entry['seed'],entry['config'])
    if entry['checkpoint']:
...
def run_suite(specs,vectors,cfg,out,validation_cases,test_cases):
    ...
    for spec in specs:
        ...
        for seed in seeds:
            ...
            entry=train_policy(spec,vectors,cfg,folder/'training',validation_cases,seed)
            registry.append(entry);dump(out/'model_registry.json',registry)
            records+=evaluate_policy(entry,vectors,cfg,test_cases,folder/'test')
            write_csv(out/'episode_metrics.csv',records)
