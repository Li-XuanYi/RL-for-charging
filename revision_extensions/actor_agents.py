...
class ActorPolicy:
    """CPU actor-critic policy with isolated sampling and checkpointed RNG state."""
    ...
    def __init__(self, method, n_agents, seed, config):
        ...
        self.update_count = 0
        self._identities = torch.eye(self.n_agents, dtype=torch.float32)
    ...
    def _rng_scope(self):
        # fork_rng restores the process RNG on exit. Each policy advances only
        # its own CPU stream; deterministic evaluation does not consume it.
    ...
    def _init_mappo(self):
        ...
        self.actor = _mlp(3 + self.n_agents, output, self.width)
        self.critic = _mlp(3 * self.n_agents, 1, self.width)
    ...
    def _init_sac(self):
        ...
        self.actor = _mlp(central_inputs, 2 * self.n_agents, self.width)
        self.q1 = _mlp(central_inputs + self.n_agents, 1, self.width)
        self.q2 = _mlp(central_inputs + self.n_agents, 1, self.width)
        ...
        self.log_alpha = torch.tensor(math.log(alpha), requires_grad=self.auto_alpha)
    ...
    def _bounds(self, masks):
        ...
        ids = torch.arange(self.n_actions, dtype=torch.long)
    ...
    def _episode(self, episode):
        ...
        actions = torch.as_tensor(np.asarray(episode["actions"]), dtype=torch.float32)
        done = torch.as_tensor(np.asarray(episode["done"]), dtype=torch.float32).reshape(-1)
        ...
        return obs, masks, actions, torch.as_tensor(rewards), done
    ...
    def _learn_mappo(self, episodes):
        ...
        for episode in episodes:
            ...
            old_log_probs = torch.as_tensor(np.asarray([info["log_probs"] for info in infos]), dtype=torch.float32)
            old_values = torch.as_tensor([float(info["value"]) for info in infos], dtype=torch.float32)
    ...
    def save(self, path, metadata=None):
        ...
        payload = {"schema": 1, "method": self.method, "n_agents": self.n_agents,
                   ...
                   "np_rng": self.np_rng.bit_generator.state, "torch_rng": self.torch_rng_state,
    ...
    def load(self, path):
        ...
        self.torch_rng_state = payload["torch_rng"].clone()
