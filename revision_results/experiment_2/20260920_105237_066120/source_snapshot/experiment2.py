...
def _handoff(root, out, cfg):
    ...
    if cfg.get('experiment1_result'):
        ...
        if not pointer.exists():
            ...
            if started.exists():
                recent=inside(Path(started.read_text(encoding='utf-8-sig').strip()),base)
                summary_file=recent/'summary.json'
                if summary_file.exists():
                    quality=json.loads(summary_file.read_text(encoding='utf-8-sig')).get('quality',{})
                    if quality.get('computation_completed') and not quality.get('numerical_precision_passed'):
                        raise RuntimeError('实验 1 的拟合已结束，但网格/求解精度检查未通过，因此尚未生成 handoff.json。'
                            '请先运行 run_experiment_1_recheck.cmd，复核通过后再运行实验 2。'
                            f'原结果保留在：{recent}。不能通过手工创建完成标记绕过精度检查。')
