...
if defined REPRO_PYTHON (
  "%REPRO_PYTHON%" -u "%~dp0revision_gpu\bootstrap_gpu.py" 6 %*
) else if exist "%~dp0.venv-reproduce\Scripts\python.exe" (
  "%~dp0.venv-reproduce\Scripts\python.exe" -u "%~dp0revision_gpu\bootstrap_gpu.py" 6 %*
) else (
  python -u "%~dp0revision_gpu\bootstrap_gpu.py" 6 %*
)
