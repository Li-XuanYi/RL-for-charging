...
class QMIX:
    ...
    def save_model(self, path):
        # 保存模型参数，包括 DRQN 网络和 QMIX 网络
        torch.save({
            'eval_drqn_net': self.eval_drqn_net.state_dict(),  # 保存 eval DRQN 网络权重
            'target_drqn_net': self.target_drqn_net.state_dict(),  # 保存 target DRQN 网络权重
            'eval_qmix_net': self.eval_qmix_net.state_dict(),  # 保存 eval QMIX 网络权重
            'target_qmix_net': self.target_qmix_net.state_dict(),  # 保存 target QMIX 网络权重
            'optimizer': self.optimizer.state_dict(),  # 保存优化器状态
            'eval_hidden': self.eval_hidden,  # 保存隐藏状态
            'target_hidden': self.target_hidden  # 保存 target 网络隐藏状态
        }, path)
    ...
    def load_model(self, path):
        ...
        self.eval_drqn_net.load_state_dict(checkpoint['eval_drqn_net'])  # 恢复 eval DRQN 网络权重
        self.target_drqn_net.load_state_dict(checkpoint['target_drqn_net'])  # 恢复 target DRQN 网络权重
        self.eval_qmix_net.load_state_dict(checkpoint['eval_qmix_net'])  # 恢复 eval QMIX 网络权重
        self.target_qmix_net.load_state_dict(checkpoint['target_qmix_net'])  # 恢复 target QMIX 网络权重
        self.optimizer.load_state_dict(checkpoint['optimizer'])  # 恢复优化器状态
        self.eval_hidden = checkpoint['eval_hidden']  # 恢复 eval 网络隐藏状态
        self.target_hidden = checkpoint['target_hidden']  # 恢复 target 网络隐藏状态
