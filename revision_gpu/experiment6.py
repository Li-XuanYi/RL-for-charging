"""E6: separately trained pack sizes, CPU/CUDA profiling, and evidence-chain closure.
...
from resources import MemoryProbe
...
def _benchmark(policy, n_agents, cfg, label, trained, checkpoint=None):
    ...
    if len(inventory['parameter_devices'])!=1:raise ValueError('Benchmark requires one consistent network device')
    ...
    return {
        'method_id': label, 'n_agents': int(n_agents), 'trained': bool(trained),'device':str(device),
        ...
        'scope': 'Synchronized policy.act wall latency on the separately labelled CPU or CUDA device; host/device transfers included. Policy input construction plus actor inference '
...
def _profile_entry(entry,n_agents,cfg,out):
    results=[]
    for device in cfg.get('benchmark_devices',['cpu',cfg.get('device','cpu')]):
        policy=load_policy(entry,device=device)
        with MemoryProbe(device) as monitor:
            row=_benchmark(policy,n_agents,cfg,entry['method_id'],True,entry['checkpoint'])
        row.update(seed=entry['seed'],resource_usage=monitor.result)
        dump(Path(out)/f'policy_timing_{device.replace(":","_")}.json',row)
        results.append(row);del policy
...
def _closure(out, ctx, cfg, timing, training_cost, record_count):
    ...
    slice_evidence=out/'e2_completed_90s_evidence.json'
    if slice_evidence.exists():
        upstream['experiment_2']={'source':str(slice_evidence),'sha256':sha256(slice_evidence),
                                 'content':json.loads(slice_evidence.read_text(encoding='utf-8-sig'))}
    evidence = [
        ...
        {'reviewer': 'R2.4', 'concern': 'scalability and computation',
         'evidence': 'E6 N=3/6/12 separately trained closed-loop experiments and large-N synthetic CPU/CUDA actor timing',
    ...
    lines = [
        ...
        '当前训练成本来自并行负载，不是独占设备测量；仅相同机器与线程设置仍不足以排除竞争。'
        '需要独占条件的训练成本比较时，将 MAX_WORKERS 设为 1 单独运行。',
        'inference_timings.csv 的纯策略计时使用合成观测：已训练 N=3/6/12 '
        '与较大规模未训练网络分开标注。它不包含物理模型、安全筛选、通信与硬件延迟。'
        '大规模未训练网络的速度不能作为控制成功率或泛化结果。',
        'CPU/CUDA 计时分列，CUDA 调用显式同步并包含主机/显卡传输。'
        '本项目训练进程结束后才开始推理计时，其他用户的后台任务仍可能影响结果。'
        'RSS 为采样峰值；CUDA 显存为本进程 PyTorch 分配器口径，排除驱动和其他作业。',
...
def run(root: Path, out: Path, cfg: dict):
    ...
    dump(out / 'machine.json', {
        ...
        'cpu_threads': torch.get_num_threads(), 'training_device':effective['device'],
        'benchmark_devices':effective.get('benchmark_devices',['cpu',effective['device']]),
    ...
    for method in ('qmix', 'mappo'):
        ...
        if len(rates) != 1:
            raise ValueError(f'E6 needs one E3 declared learning rate '
    ...
    protocol = {
        ...
        'training_cost_scope': 'Per-process memory and wall time under the recorded parallel workload, not isolated algorithmic scaling; use MAX_WORKERS=1 for isolated training-cost runs.',
        'inference_timing_isolation': 'All project training workers exit before serial CPU/CUDA benchmarks; other server jobs remain uncontrolled.',
        'hyperparameter_source': 'Each method retains its E3 declared '
        ...
        'inference_scope': 'Separately synchronized CPU/CUDA act() on fixed synthetic observations; no claim '
    ...
    for n_agents in sizes:
        ...
        for spec in specs:
            for seed in seeds:
                ...
                jobs.append(train_job(spec,plant_vectors,effective,directory/'training',validation,seed,tests,directory/'test'))
    completed=execute_jobs(jobs,out/'parallel_scaling_jobs',effective)
    # All simulation workers have exited before resource benchmarks start.
    for result in completed:
        entry=result['entry'];registry.append(entry)
        n_agents=entry['n_agents'];seed=entry['seed'];spec=entry['spec']
        directory=out/f'N{n_agents}'/spec['id']/f'seed_{seed}'
        training_s=result['training_s'];evaluation_s=result['evaluation_s'];evaluated=result['records']
        ...
        cost = {'n_agents': n_agents, 'method_id': spec['id'], 'seed': seed,
                ...
                'worker_gpu_uuid':result['worker_gpu'],
                'training_cost_context':'parallel workload; not isolated scaling latency',
                'evaluation_wall_s': evaluation_s,
                'evaluated_episodes': len(evaluated)}
        cost.update(entry.get('resource_usage',{}))
        ...
        timing.extend(_profile_entry(entry,n_agents,effective,directory))
    ...
    for n_agents in timing_sizes:
        for spec in specs:
            ...
            for device in effective.get('benchmark_devices',['cpu',effective['device']]):
                policy=make_policy({**spec,'device':device},n_agents,seed,{**effective,'device':device})
                with MemoryProbe(device) as monitor:
                    measurement=_benchmark(policy,n_agents,effective,spec['id'],False)
                measurement.update(seed=seed,resource_usage=monitor.result)
                timing.append(measurement)
                dump(out/'inference_only'/f'N{n_agents}'/f'{spec["id"]}_{device.replace(":","_")}.json',measurement)
                del policy
                if torch.device(device).type=='cuda':torch.cuda.empty_cache()
    write_csv(out / 'inference_timings.csv',
