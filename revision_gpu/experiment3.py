...
def run(root:Path,out:Path,cfg:dict):
    ...
    tune=bool(effective.get('tune_learning_rates',True))
    ...
    if tune and (not tuning_seeds or len(set(tuning_seeds))!=len(tuning_seeds) or set(tuning_seeds)&set(effective['seeds'])):
    ...
    protocol['learning_rate_search_enabled']=tune
    if not tune:
        protocol['tuning']={'enabled':False,'learning_rate':effective['learning_rate'],
            'scope':'One predeclared learning rate for all learned methods; zero search trials. Short-budget exploration, not individually tuned optimal baselines.'}
    ...
    for method in LEARNED:
        if not tune:
            selected={'id':method,'method':method,'learning_rate':effective['learning_rate']}
            specs.append(selected);chosen[method]={'spec':selected,'selection':'predeclared fixed learning rate; no search'}
            dump(out/'selected_hyperparameters.json',chosen)
            continue
        candidates=[]
    ...
    result=finalize(out,suite['records'],{'experiment':3,'formal_seed_count_met':len(effective['seeds'])>=5,
        'learning_rate_search_enabled':tune,'tuning_is_validation_only':True,'discrete_continuous_separated':True,
