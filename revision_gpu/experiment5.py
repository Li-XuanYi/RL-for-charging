...
from parallel_jobs import train_job,evaluation_job,execute_jobs
...
def _sensitivity_specs(learning_rate,dqn_learning_rate):
    ...
    for component in ('time','balance', 'voltage', 'temperature'):
    ...
    for epsilon,label in ((.5,'dqn_epsilon_reference'),(1.,'dqn_epsilon_one')):
        specs.append(dict(deepcopy(common),id=label,method='dqn',learning_rate=dqn_learning_rate,epsilon_start=epsilon))
    return specs
...
def run(root: Path, out: Path, cfg: dict):
    ...
    for entry in upstream_frozen:
        if sha256(project_path(entry['checkpoint'])) != entry['checkpoint_sha256']:
    ...
    dqn_rates={float(e['config']['learning_rate']) for e in ctx['registry'] if e['method_id']=='dqn'}
    if len(dqn_rates)!=1:raise ValueError('E5 needs one E3 DQN learning-rate setting')
    specs = _sensitivity_specs(qmix_learning_rate,next(iter(dqn_rates)))
    ...
    protocol = {
        ...
        'learning_rate_source': 'Each method retains its E3 declared setting: QMIX for reward/domain groups, DQN for DQN exploration controls.',
    ...
    sensitivity_records, trained, jobs = [], [], []
    for spec in specs + [domain_spec]:
        for seed in seeds:
            directory = out / 'training' / spec['id'] / f'seed_{seed}'
            jobs.append(train_job(spec,vectors,effective,directory,validation,seed,sensitivity_tests,
                                  out/'sensitivity'/spec['id']/f'seed_{seed}'))
    for result in execute_jobs(jobs,out/'parallel_sensitivity_jobs',effective):
        trained.append(result['entry'])
        for row in result['records']:row['study_panel']='reward_and_exploration'
        sensitivity_records.extend(result['records'])
    dump(out/'model_registry.json',trained)
    write_csv(out/'sensitivity_episode_metrics.csv',sensitivity_records)

    compare([r for r in sensitivity_records if r['method']=='qmix'], out / 'sensitivity_statistics', effective,
            references={'discrete': 'qmix_reward_reference'})
    compare([r for r in sensitivity_records if r['method']=='dqn'],out/'dqn_exploration_statistics',effective,
            references={'discrete':'dqn_epsilon_reference'})
    ...
    jobs=[evaluation_job(entry,vectors,effective,transfer_tests,
                        out/'generalization'/_method(entry)/f'seed_{entry["seed"]}') for entry in frozen]
    for result in execute_jobs(jobs,out/'parallel_generalization_jobs',effective):
        records=result['records'];checkpoint_audit.append(result['audit'])
        for row in records:
